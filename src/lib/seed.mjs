...
async function main() {
  ...
  await prisma.task.createMany({
    data: [
      { title: 'Revisar órdenes pendientes', description: 'Verificar todas las órdenes en estado pendiente del día', dueDate: new Date(now.getTime() + 86400000), userId: manager.id, emailAlert: true },
      { title: 'Contactar cliente Walmart', description: 'Seguimiento de orden con shortage', dueDate: new Date(now.getTime() + 172800000), userId: employee1.id, emailAlert: false },
      { title: 'Reporte semanal', description: 'Generar y enviar reporte semanal de operaciones', dueDate: new Date(now.getTime() + 432000000), userId: admin.id, emailAlert: true },
    ],
    skipDuplicates: true,
  });
